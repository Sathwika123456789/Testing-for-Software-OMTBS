...
// Check if $_SESSION['theatre'] is set and not empty
if (isset($_SESSION['theatre']) && !empty($_SESSION['theatre'])) {
  $th = mysqli_query($con, "select * from tbl_theatre where id = '" . $_SESSION['theatre'] . "'");
  if ($th) {
    $theatre = mysqli_fetch_array($th);

    // Check if the 'name' key exists in the $theatre array
    if (isset($theatre['name'])) {
      $theatreName = $theatre['name'];
    } else {
      // Handle the case where 'name' key doesn't exist
      $theatreName = "Theatre Name Not Found";
    }
  } else {
    // Handle the database query error
    $theatreName = "Error Retrieving Theatre Name";
  }
} else {
  // Handle the case where $_SESSION['theatre'] is not set or empty
  $theatreName = "Theatre Not Selected";
}
?>
